...
\begin{definition}[]
    Define f,D,$\bar{x}$, D is an open set, suppose $\bar{x}$ is a local minmum for f on D,
    Then:
    \begin{enumerate}
        \item Nec: $\nabla f(\bar{x})=0,\nabla^2f(\bar{x}) \geq 0
        $ positive semidefinite
        \item Suff: $\nabla f(\bar{x})=0,\nabla^2f(\bar{x}) > 0
        $ positive definite $\implies \bar{x}$ is a strict local minmum on Define
...
\begin{Example}[Algorithm to find Min using Nec above]
    Given f:$\mathbb{R}\implies \mathbb{R}$ and $f^{'}(\bar{x})\neq 0$
    , then $x_{new}=\bar{x}-slop*f^{'}(\bar{x})$.
