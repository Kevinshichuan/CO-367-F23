...
\begin{definition}[General NLO/NLP]
    ...
    $$\underbrace{p^*}_{\text{Optimal Value}} = \min\;\;\;\; \underbrace{f(x)}_{Objective function}$$
    s.t.
    \begin{align*}
...
\begin{Example}[Max-cut Problem]
    ...
    $$\max\;\;\;\; \frac{1}{2} \sum_{ij \in E} w_{ij}(1 - x_ix_j)$$
...
\subsection{Lecture 2}
\begin{Example}[Simplest Case - No Constraints]
...
\begin{theorem}[Taylor's Theorem on the real line]
    ...
    or equivalently
    $$f(\bar x + \delta x) = \underbrace{f(x) + f'(x) \delta x}_{\text{Linear approximation}} + O(|\delta x|)$$
\end{theorem}
